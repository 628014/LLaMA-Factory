...
def extract_index_from_response(response_text):
    ...
    if match:
        ...
        # 为了减少日志刷屏，这里注释掉或者只打印极少量的warning
...
def process_single_item(item, feature_cache):
    ...
    try:
        ...
        # 4. 计算相似度并寻找 Bad Choose (Easy Negative / Lowest Similarity)
        # ================= 修改区域 Start =================
        # 逻辑：在所有非 Target 图片中，找到与 Target 相似度【最低】的那张
        min_sim = 100.0  # 初始值设为一个很大的数 (余弦相似度最大为1)
        bad_choose_idx = -1
