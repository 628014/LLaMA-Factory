...
"""
使用vLLM推理CUHK-PEDES数据集，并将结果保存到JSON文件中
在这之前需要执行 infer_cuhk_data_process.py 生成与输入匹配到数据文件
环境在conda activate vllm_infer 中 
5w 1h推理完，没有使用原生的llama-factory 多卡分布式，仅使用vLLM自带的多GPU支持
...
# 2. 数据路径
DATA_PATH = "/root/home/wangrui/code/LLaMA-Factory/data/CUHK-PEDES_instruct_formatted_train_and_test_all.json"

# 3. 输出路径
OUTPUT_PATH = "/root/autodl-tmp/saves/qwen2_5vl-7b/lora/pre_retpreid_with_score/lora64_full_cuhk_train_and_test_all/prediction_results.json"
