...
"""
This script processes the CUHK-PEDES dataset and formats it for instruction-based inference.
"""

# ================= 配置区域 =================
# 输入文件：CUHK-PEDES 的原始 json
INPUT_FILE = '/root/autodl-tmp/MLLM4Text-ReID-main/data/CUHK-PEDES/CUHK-PEDES/reid_raw.json'
# 输出文件：格式化后的 json
OUTPUT_FILE = '/root/home/wangrui/code/LLaMA-Factory/data/CUHK-PEDES_instruct_formatted_train_and_test_all.json'
# 图片前缀：根据你的要求指向 imgs 目录
...
def process_batch(data_chunk):
    ...
    for item in data_chunk:
        ...
        # 拼接绝对路径: Prefix + train_query/p10714_s14595.jpg
        full_image_path = os.path.join(IMG_PREFIX, clean_rel_path)
        
        # 2. Captions 处理 (核心修改：合并而非遍历)
        captions = item.get('captions', [])
        
        if not captions:
            continue # 如果没有描述则跳过
            
        # 将列表中的多条描述合并成一个字符串，中间用空格隔开
        merged_caption = " ".join(captions)
        
        # 3. 替换模板
        user_content = PROMPT_TEMPLATE.replace('{caption}', merged_caption)
        
        # 4. 构建数据 (每个item只生成一条数据)
        new_entry = {
            "messages": [
                {
